...
trip.data$member_age_group[trip.data$member_age > 60] = "Senior"
```
...
ggplot(data = trip.data2, aes(x = age, fill = member_gender)) + 
      ...
      labs(title = "Different Distributions for Different Subscriber Types",
       x = "Age",
       y = "Density",
       fill = "Gender") + scale_fill_manual(values=c("dark blue", "dark green", "#56B4E9"))
```
...
ggplot(data = df.1, aes(x = day, y = hour, fill = count.rides)) + 
  ...
  labs(title = "Bike Usage Focused on Weekdays",
       x = "Day of Week",
       y = "Hour of Day",
...
day_age_rides %>%
  ...
  mutate(total_duration = total.rides * avg.duration, 
         hour = factor(hour, levels = (0:23)),
         age_general = cut(age, c(0, 24, 34, 65, Inf), labels = c("Young Adult", "Middle Adult", "Middle Age", "Senior"))) %>%
...
ggplot(data = df.5, aes(x = hour, y = count.rides, col = member_gender)) + 
  geom_jitter(alpha = .8) +
  scale_color_manual(values = c("pink", "skyblue", "darkgrey", "blue")) +
  labs(title = "Number of Rides Each Hour of the Day",
       subtitle = "By Gender",
       x = "Hour",
       y = "Number of Rides",
